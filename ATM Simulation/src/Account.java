...
class Account {
    ...
    private static Map<Integer, Account> accountStorage = new HashMap<>();     // HashMap for all accounts

    // Constructors

    // No-arg for making initial account object.
    public Account() {
    }

    // Used for manually created standard accounts. Default names changed later.
    ...
    // Used for creating an admin account. Default names changed later.
    ...
    public boolean add(int id, boolean isAdmin) {
        accountStorage.put(0, new Account(0, adminAcc));
        return true;
    }
